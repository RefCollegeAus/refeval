...
export default function Home() {
  ...
  const [timelineWindowExtend, setTimelineWindowExtend] = useState(0);
  ...
  function resetDrafts() {
    setDraftRefereeTarget("All Referees"); setDraftExtraOfficials([]); setDraftOutcome(""); setDraftCategoryGroup(""); setDraftSpecificTag(""); setDraftCategory(""); setDraftPosition(""); setDraftCoverage(""); setDraftNotes(""); setDraftIsLearningClip(false); setDraftEndSeconds(null); setTimelineWindowExtend(0); setCodingError("");
  }
  ...
  function buildTag(baseSeconds: number, tagMode: Mode, patch: Partial<CodedTag>): CodedTag {
    ...
    // startTimeSeconds = explicit 10 s pre-roll saved alongside the incident time
    const startTimeSeconds = Math.max(0, adjustedSeconds - 10);
  ...
  function openVideoCoding() {
    ...
    setEditingTagId(null); setShouldResumeVideo(wasPlaying); setCodingSecond(Math.max(0, current)); resetDrafts(); setWizardStep(1);
  // Seek to clip start so the educator sees context before the incident
  const _offset = Number(activeReview?.timestampOffset || 0);
  const _startAdj = Math.max(0, Math.max(0, current + _offset) - 10);
  if (usingYouTubeVideo && youtubePlayerRef.current?.seekTo) { youtubePlayerRef.current.seekTo(_startAdj, true); }
  else if (videoRef.current) { videoRef.current.currentTime = _startAdj; }
  setCodingOpen(true);
  }
  ...
  function openEditTag(tag: CodedTag) {
    ...
    setDraftOutcome(tag.outcome || ""); setDraftPosition(tag.position || ""); setDraftCoverage(tag.coverage || ""); setDraftNotes(tag.notes || ""); setDraftIsLearningClip(tag.isLearningClip ?? false); setDraftEndSeconds(tag.endTimeSeconds ?? null);
    ...
    setWizardStep(1);
    // Seek to resolved clip start so step 1 shows context before the incident
    const _bounds = resolveClipBounds(tag);
    if (usingYouTubeVideo && youtubePlayerRef.current?.seekTo) { youtubePlayerRef.current.seekTo(_bounds.startTime, true); }
    else if (videoRef.current) { videoRef.current.currentTime = _bounds.startTime; }
    setCodingOpen(true);
  }
