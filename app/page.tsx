...
export default function Home() {
  ...
  const videoPanStartRef                    = useRef<{ sx: number; sy: number; px: number; py: number } | null>(null);
  const [videoOverlayEl, setVideoOverlayRef] = useState<HTMLDivElement | null>(null);
  ...
  function resetDrafts() {
    setDraftRefereeTarget("All Referees"); setDraftExtraOfficials([]); setDraftOutcome(""); setDraftCategoryGroup(""); setDraftSpecificTag(""); setDraftCategory(""); setDraftPosition(""); setDraftCoverage(""); setDraftNotes(""); setCodingError("");
  }
  ...
  // Ctrl/Meta key state — activates the zoom overlay while held
  useEffect(() => {
    const down = (e: KeyboardEvent) => { if (e.key === "Control" || e.key === "Meta") setCtrlHeld(true); };
    const up   = (e: KeyboardEvent) => { setCtrlHeld(false); };
    window.addEventListener("keydown", down);
    window.addEventListener("keyup", up);
    return () => { window.removeEventListener("keydown", down); window.removeEventListener("keyup", up); };
  }, []);

  // Non-passive wheel listener on zoom overlay — prevents browser page-zoom on Ctrl+scroll
  useEffect(() => {
    ...
    const handler = (e: WheelEvent) => {
      ...
      e.preventDefault();
      setVideoZoom(prev => {
        const next = Math.round(Math.max(1, Math.min(2, prev + (e.deltaY < 0 ? 0.1 : -0.1))) * 10) / 10;
        if (next <= 1) setVideoPan({ x: 0, y: 0 });
        return next;
      });
    };
    videoOverlayEl.addEventListener("wheel", handler, { passive: false });
    return () => videoOverlayEl.removeEventListener("wheel", handler);
  }, [videoOverlayEl]);
  ...
  function openEditTag(tag: CodedTag) {
    ...
    setDraftOutcome(tag.outcome || ""); setDraftPosition(tag.position || ""); setDraftCoverage(tag.coverage || ""); setDraftNotes(tag.notes || "");
