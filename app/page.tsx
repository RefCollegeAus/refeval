...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const viewerForbidden: Screen[] = ["educator", "referee", "reviewer", "refereeReview", "comment-inbox", "referee-stats", "database", "org-settings", "clip-library", "playlists", "playlist-detail", "team-management", "assignments", "assignment-detail", "learning-hub", "learning-progress", "groups"];
  ...
  if (screen === "comment-inbox") {
    return (
      <main>
        <Header session={session} onHome={() => setScreen("educator")} onAdmin={() => setScreen("database")} onLearning={() => setScreen("learning-hub")} onProfile={() => setScreen("user-profile")} onLogout={logout} />
  ...
  if (screen === "learning-progress" && session) {
    return (
      <main>
        <Header session={session} onHome={() => setScreen("learning-hub")} onAdmin={() => setScreen("database")} onLearning={() => setScreen("learning-hub")} onProfile={() => setScreen("user-profile")} onLogout={logout} />
        <LearningProgress
          session={session}
          assignments={assignments}
          members={members}
  ...
  if (screen === "groups" && session) {
    return (
      <main>
        <Header session={session} onHome={() => setScreen(returnToScreen)} onAdmin={() => setScreen("database")} onLearning={() => setScreen("learning-hub")} onProfile={() => setScreen("user-profile")} onLogout={logout} />
        <GroupsScreen
          session={session}
          groups={groups}
          members={members}
  ...
  if (screen === "referee") {
    ...
    return (
      <main>
        <Header session={session} onHome={() => setScreen(session?.activeRole === "referee" ? "referee" : session?.activeRole === "viewer" ? "viewer" : "educator")} onAdmin={() => setScreen("database")} onLearning={() => setScreen("learning-hub")} onProfile={() => setScreen("user-profile")} onLogout={logout} />
