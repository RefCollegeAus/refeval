...
const PLAYBACK_RATES = [1.5, 1.25, 1, 0.5, 0.25];
...
export default function Home() {
  ...
  function openEditTag(tag: CodedTag) {
    ...
    // A previous wizard session (this tag or another) may have left a validation error set
    // (e.g. an abandoned "Please select a Coverage area." from saveVideoCode) — without this,
    // that stale message would show immediately on Step 1 of an otherwise-complete tag.
    setCodingError("");
    setWizardStep(1); setCodingOpen(true);
  }
  ...
  // Shared selectable-option button treatment for the tagging wizard's Outcome/Category/
  // Position/Coverage/Referee steps — was five copies of the legacy .wizard-opt CSS class
  // (no focus-visible ring at all), now one place expressing the same affordance via the
  // app's tokens. `compact` matches the smaller .wizard-opt--sm variant (specific category
  // tags, extra officials). Selected/unselected colour is passed in — the Outcome step
  // colours itself per-outcome (see WIZARD_OPT_UNSELECTED/oc below) rather than the plain
  // accent highlight every other step uses.
  function wizardOptClass(selected: boolean, compact?: boolean) {
    return cn(
      "cursor-pointer rounded-xl border text-left font-bold leading-[1.3] transition-colors",
      "focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent",
      compact ? "rounded-[10px] px-2.5 py-[9px] text-[13px]" : "px-4 py-[13px] text-[15px]",
      selected
        ? "border-accent bg-accent/[.18] text-text"
        : "border-border bg-panel-2 text-text hover:border-accent/50 hover:bg-accent/[.07]",
    );
  }
  ...
  if (screen === "viewer" && session) {
    return (
      <AppShell
        ...
        navContext={navContext} activeOrgPage={orgPage}
  ...
  return <AppShell session={session} activeScreen={screen} onHome={() => setScreen(session?.activeRole === "referee" ? "referee" : session?.activeRole === "viewer" ? "viewer" : "educator")} onAdmin={() => setScreen("database")} onOrganisation={() => setScreen("organisation")} onLearning={() => setScreen("learning-hub")} onNavigate={navigate} orgLogoUrl={orgSettings.settings.branding.logoUrl} navContext={navContext} activeOrgPage={orgPage} onProfile={() => setScreen("user-profile")} onNotifications={() => setScreen("notifications")} unreadNotificationCount={visibleUnreadCount} onLogout={logout} hideSidebar contentClassName="p-3 lg:p-4">
    ...
    <TaggedClipsModal
      ...
      activeCommentTagId={activeCommentTagId}
      onToggleComments={tagId => { setSelectedTagId(tagId); setActiveCommentTagId(t => t === tagId ? null : tagId); }}
