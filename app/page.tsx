...
export default function Home() {
  ...
  async function saveVideoCode() {
    ...
    if (!draftCategory) { setWizardStep(4); setCodingError("Please select a Call Category."); return; }
    setCodingError("");
    const patch = {
  ...
  if (screen === "comment-inbox") {
    return (
      <main>
        <Header session={session} onHome={() => setScreen("educator")} onAdmin={() => setScreen("database")} onProfile={() => setScreen("user-profile")} onLogout={logout} />
