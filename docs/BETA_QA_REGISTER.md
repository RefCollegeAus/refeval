...
| QA-019 | Medium | All | Data Scoping | View-Only Games, Reviews, and other data hooks | Active organisation scoping: multi-organisation users may receive legitimately accessible records from all organisations because several data hooks rely on RLS membership rather than filtering by `session.activeOrganisation.id`. This is a correctness/isolation-of-context issue, not currently identified as an unauthorized-access vulnerability. Review systematically in a dedicated future pass. | User holds memberships in 2+ organisations | Only the active organisation's records are shown | Records from all member organisations may be mixed together in one view | Identified — Known Limitation (not a vulnerability) | — |
