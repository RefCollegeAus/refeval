...
| QA-018 | High | All | Security | RLS on reviews and clips | `reviews` and `clips` tables return all rows to anon-key-only requests (no user JWT) — suggests RLS may be disabled on these tables | Request `GET /rest/v1/reviews` with only the publishable API key (no user session) | 0 rows or 401 (unauthenticated access blocked) | All rows returned — 4 reviews, 199 clips visible without authentication | **Unresolved** — cannot confirm RLS status without direct SQL access; verify in Supabase Dashboard → Table Editor → RLS. If disabled, run: `ALTER TABLE public.reviews ENABLE ROW LEVEL SECURITY; ALTER TABLE public.clips ENABLE ROW LEVEL SECURITY;` in SQL Editor | — |
