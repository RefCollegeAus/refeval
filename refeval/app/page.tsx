  ...
  async function startNewReview() {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        game: review.game,
        title: review.game,
        organisation_id: "300e750e-8c45-4026-b4d0-4caadd1ded7d",
...
if (error) {
  console.error("Save review meta error:", error);
  alert(error.message);
  return;
}
...
setReviews(items =>
  items.map(review => {
    ...
    return {
      ...review,
      game: reviewGame,
      referee1Id: reviewRef1,
      referee2Id: reviewRef2,
      referee3Id: reviewRef3,
      referee1Name: r1?.name || "",
      referee2Name: r2?.name || "",
      referee3Name: r3?.name || "",
    ...
    async function saveCompleteLater() {
      await saveReviewMeta("In Review");
      setScreen("educator");
    }
    async function submitReview() {
      await saveReviewMeta("Completed");
      setScreen("educator");
    }
    async function deleteReview(id: string) {
      ...
      if (clipError) {
        console.error("Delete clips error:", clipError);
      ...
      if (reviewError) {
        console.error("Delete review error:", reviewError);
      ...
      return (
        <main>
          <Header
            ...
            onHome={() => setScreen(currentUser?.role === "referee" ? "referee" : "educator")}
    ...
    >
      Delete
    </button>}><Trash2 size={16} /> Clear Tags</button></div><table><thead><tr><th>Time</th><th>Referees</th><th>Link</th><th>Mode</th><th>Outcome</th><th>Coverage</th><th>Position</th><th>Category</th><th>Comments</th><th></th></tr></thead><tbody>{reviewTags.map(tag => <tr key={tag.id}><td><button onClick={() => jump(tag.adjustedSeconds)}>{tag.adjustedTime}</button></td><td><strong>{slotName(tag.refereeTarget, activeReview)}</strong> <span className="hint">(Call)</span><br />{(tag.extraReviewOfficials || []).map(s => <span className="chip" key={s}>{slotName(s, activeReview)} Review</span>)}</td><td>{tag.timestampLink ? <a href={tag.timestampLink} target="_blank">Open timestamp</a> : <span className="hint">No link</span>}</td><td>{tag.mode}</td><td>{tag.outcome}</td><td>{tag.coverage}</td><td>{tag.position}</td><td>{tag.category}</td><td>{tag.notes}</td><td>{tag.mode === "video" && <button onClick={() => openEditTag(tag)}>Edit</button>}
      <button
        className="danger"
        onClick={async () => {
          const { error } = await supabase
